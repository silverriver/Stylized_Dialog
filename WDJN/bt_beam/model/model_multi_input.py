...
class MultiInputModel(nn.Module):
    def __init__(self, config, vocab):
        ...
        self.transformer_module = TransformerModule(
            config.n_layers, len(vocab), config.n_pos_embeddings, config.embeddings_size, vocab.pad_id, config.n_heads,
            config.dropout, config.embed_dropout, config.attn_dropout, config.ff_dropout, config.n_styles)
        self.pre_softmax = nn.Linear(config.embeddings_size, len(vocab), bias=False)
        self.tie_pre_softmax()

    def tie_pre_softmax(self):
    ...
    def encode(self, x, x_len, enc_id, x_embed=None):
        return self.transformer_module(x, x_len, x_embed=x_embed, enc_id=enc_id)

    def generate(self, enc_x):
        return self.pre_softmax(enc_x)

    def decode(self, x, x_len, enc_contexts=[], x_embed=None, dec_id=None):
        x, _ = self.transformer_module(x, x_len, enc_contexts, x_embed, dec_id=dec_id)
        return self.generate(x)

    def classify(self, seq_lengths, x=None, x_embed=None):
        # return self.classifier(seq_lengths, x, x_embed)
        if x_embed is None:
            if hasattr(self.transformer_module, 'embeddings'):
                embeddings = self.transformer_module.embeddings
            else:
                embeddings = self.transformer_module.module.embeddings
            x_embed = embeddings(x)
        return self.classifier(x_embed, seq_lengths)

    def get_word_embed(self, x):
        if hasattr(self.transformer_module, 'embeddings'):
            return self.transformer_module.embeddings(x)
        else:
            return self.transformer_module.module.embeddings(x)

    def get_enc_embed(self, x):
        if hasattr(self.transformer_module, 'enc_embeddings'):
            return self.transformer_module.enc_embeddings(x)
        else:
            return self.transformer_module.module.enc_embeddings(x)

    def get_dec_embed(self, x):
        if hasattr(self.transformer_module, 'dec_embeddings'):
            return self.transformer_module.dec_embeddings(x)
        else:
            return self.transformer_module.module.dec_embeddings(x)

    def sample_gumbel(self, styles, step, enc_contexts=[]):
        ...
        for i in range(1, self.config.max_seq_len):
            decode_logits = self.decode(x, enc_contexts, x_embed)[:, -1, :]
    ...
    def predict(self, contexts=[], enc_id=None, dec_id=None):
        enc_contexts = [self.encode(c, c_len, enc_id) for c, c_len in contexts]
        prediction = self.beam_search(enc_contexts, dec_id=dec_id)
        return prediction

    def predict_beam(self, contexts=[], enc_id=None, dec_id=None):
        enc_contexts = [self.encode(c, c_len, enc_id) for c, c_len in contexts]
        prediction = self.beam_search(enc_contexts, return_beams=True, dec_id=dec_id)
        return prediction
    ...
    def beam_search(self, enc_contexts=[], return_beams=False, dec_id=None):
        with torch.no_grad():
            if len(enc_contexts) == 0:
                return []
            ...
            prevs = dec_id.reshape(-1, 1).repeat(1, self.config.beam_size).view(-1, 1)
            prev_embed = self.get_dec_embed(prevs)
            ...
            for i in range(self.config.max_seq_len):
                outputs, _ = self.transformer_module(None, beam_lens.view(-1), beam_enc_contexts, prev_embed, dec_id=prevs[:, 0])
                ...
                prev_embed = torch.cat([prev_embed, self.get_word_embed(sym_idxs)], dim=1)
            ...
            predicts = []
            result = prevs.view(batch_size, self.config.beam_size, -1)   # [bs, beam_size, seq_len]
    ...
    def beam_search_train_bt(self, enc_contexts=[], dec_id=None):
        with torch.no_grad():
            if len(enc_contexts) == 0:
                return []
            ...
            for i in range(self.config.max_resp_len):
                outputs, _ = self.transformer_module(None, beam_lens.view(-1), beam_enc_contexts, prev_embed, dec_id=prevs[:, 0])
    ...
    def top_k_top_p_search(self, enc_contexts=[], top_k=0, top_p=0.0, filter_value=-float('Inf'),
                           sample_size=5, temperature=0.8, dec_id=None, max_len=None):
        with torch.no_grad():
            # if len(enc_contexts) == 0:
            #    return []
            assert dec_id is not None

            batch_size = dec_id.shape[0]
            device = next(self.parameters()).device

            prevs = dec_id.unsqueeze(1).repeat(1, sample_size).view(-1, 1)
            prevs_embed = self.get_dec_embed(prevs)
            ...
            for i in range(max_len):
                outputs, _ = self.transformer_module(None, lens, beam_enc_contexts, x_embed=prevs_embed, dec_id=prevs[:, 0])
                logits = self.generate(outputs[:, -1, :]) / temperature
                filtered_logits = top_k_top_p_filtering(logits, top_k=top_k, top_p=top_p)
                probabilities = F.softmax(filtered_logits, dim=-1)
                next_token = torch.multinomial(probabilities, 1).view(-1)

                next_token[is_end] = self.vocab.pad_id
                lens[~is_end] += 1
                is_end[next_token == self.vocab.eos_id] = 1
                prevs = torch.cat((prevs, next_token.view(-1, 1)), dim=-1)
                prevs_embed = torch.cat([prevs_embed, self.get_word_embed(next_token.view(-1, 1))], dim=1)
