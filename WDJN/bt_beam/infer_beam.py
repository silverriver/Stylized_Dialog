...
parser.add_argument('--out_file', help='out_file', default='infer_out_beam')
...
try:
    ...
    with torch.no_grad():
        model.eval()
        res = []
        out_file_name = os.path.join(eval_dir, args.out_file + '_' + latest_ckpt + str(args.local_rank) + '.txt' + '_lp{}'.format(config.length_penalty))
        ...
        with open(out_file_name, 'w') as f:
            ...
            for data in ITER:
                ...
                for i in range(bs):
                    tmp[i]['pred_style0'] = []
                    for pred in prediction[i]:
                        tmp[i]['pred_style0'].append(vocab.ids2string(pred))
