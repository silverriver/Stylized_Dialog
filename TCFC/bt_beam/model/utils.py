...
def load_gpt2(model, weight_file, device):
    weight_dict = torch.load(weight_file, map_location=device)
    weight_dict['style_embed.weight'] = model.transformer_module.style_embed.weight
    model.transformer_module.load_state_dict(weight_dict)
    model.transformer_module.tie_weights()
    model.eval()
