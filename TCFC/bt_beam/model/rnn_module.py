...
class ResRNNBase(nn.Module):
    ...
    def __init__(self, rnn_type, ninp, nhid, nlayers, n_embeddings, embeddings_size, padding_idx,
                 nonlinearity='tanh', dropout=0.2, direction="left2right"):
    ...
    def encode(self, x, x_prob=None):
        if x_prob is not None:
            bsz = x_prob.shape[0]
            embed = torch.matmul(x_prob, self.embeddings.weight).permute(1, 0, 2)
        else:
            bsz = x.shape[0]
            embed = self.embeddings(x).permute(1, 0, 2)
        hidden = self.init_hidden(bsz)
        hidden = repackage_hidden(hidden)
        return self.forward(embed, hidden)[0].permute(1, 0, 2)
...
class ResLSTM(ResRNNBase):
    ...
    def __init__(self, ninp, nhid, nlayers, n_embeddings, embeddings_size, padding_idx, dropout, direction):
        super(ResLSTM, self).__init__('LSTM', ninp, nhid, nlayers, n_embeddings, embeddings_size, padding_idx,
...
class ResGRU(ResRNNBase):
    ...
    def __init__(self, ninp, nhid, nlayers, n_embeddings, embeddings_size, padding_idx, dropout, direction):
        super(ResGRU, self).__init__('GRU', ninp, nhid, nlayers, n_embeddings, embeddings_size, padding_idx,
                                     dropout=dropout, direction=direction)
...
class ResRNN(ResRNNBase):
    ...
    def __init__(self, ninp, nhid, nlayers, n_embeddings, embeddings_size, padding_idx, nonlinearity='tanh', dropout=0.0, direction=False):
        super(ResRNN, self).__init__('RNN', ninp, nhid, nlayers, n_embeddings, embeddings_size, padding_idx,
                                     nonlinearity=nonlinearity, dropout=dropout,
                                     direction=direction)


if __name__ == '__main__':
    rnn = ResGRU(1000, 256, 3, 0.8, 10388, 768, 'left2right')
